...
public interface StPersonTypeRepository extends JpaRepository<StPersonTypeEntity, Long> {
    @Query("select b from StPersonTypeEntity b where b.personTypeStatus = 1 order by b.idPersonType asc")
    List<StPersonTypeEntity> findAllByStatus();

    @Query("select b from StPersonTypeEntity b where b.idPersonType=?1 and b.personTypeStatus=?2")
